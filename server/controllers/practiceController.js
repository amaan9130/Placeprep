...
import { evaluateSubmission } from '../utils/codeEvaluator.js';
...
export const submitCodingSolution = async (req, res, next) => {
  try {
    ...
    const evalResult = await evaluateSubmission(code, question.slug, language);
    ...
    if (profile) {
      ...
      if (evalResult.status === 'Accepted' && !profile.solvedCoding.some(id => String(id) === qIdStr)) {
    ...
    subObj.message = evalResult.message;

    res.status(201).json({ success: true, data: subObj });
  } catch (err) {
    next(err);
  }
};
